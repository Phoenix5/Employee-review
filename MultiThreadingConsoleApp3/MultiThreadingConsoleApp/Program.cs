...
namespace ConsoleApi
{
    class Program
    {
           ...
           Dictionary<string, string> responseLoad = new Dictionary<string, string>();
            ...
            foreach (var request in data.RequestList)
            {
                ...
                switch (request.MethodType.ToString())
                {
                    ...
                    case "post":
                        responseResult = HttpPostRequest(request.Payload, request.Url, headers);
                         payloadResponseData = (IDictionary<string, object>)responseResult.Content.ReadAsAsync<ExpandoObject>().Result;
                        foreach (var getToken in payloadResponseData)
                        {
                            if (getToken.Key == "PickupRequestID" && getToken.Value != null)
                            {
                                responseLoad.Add("PickupRequestID", getToken.Value.ToString());
                            }
                        }
                        break;
                    ...
                    case "get":
                        ...
                        responseResult = HttpGetRequest(request.Url+"'"+responseLoad["PickupRequestID"] + "'", headers);
                        break;
            ...
            var values = new[]
       {
        ...
        private static HttpResponseMessage HttpPostRequest(string payload, string url, Dictionary<string, string> headerList)
        {
            HttpClient client = new HttpClient();
        ...
        private static HttpResponseMessage HttpGetRequest(string url, Dictionary<string, string> headerList)
        {
            HttpClient client = new HttpClient();
